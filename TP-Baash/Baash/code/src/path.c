...
/**
   ...
   @return no aplica.
 */
void buscarDireccionDeArchivo(char* arch, char* paths[], char *direccion){
	int result=0;
	char searchDir[70] = "";
	char* archivo;
	...
	if(arch[0] == '/' || (arch[0] == '.' && arch[1] == '.' && arch[2] == '/')){
		...
		direccion[0] = 'X';  //bandera para indicar un nombre de programa incorrecto
		free(aux);
		return;
	}
...
void ejecutarPipeline(char* argv1[], char* argv2[], char* paths[]){
	...
	if (fork()==0) { //codigo del hijo
		...
		close(fd[1]);
		buscarDireccionDeArchivo(argv1[0], paths, executePath);
		...
		dup2(fd[0],0);
		close(fd[0]);
		buscarDireccionDeArchivo(argv2[0], paths, executePath);
